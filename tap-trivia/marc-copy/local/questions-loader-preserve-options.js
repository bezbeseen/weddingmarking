(()=>{
  ...
  function authoredDistractors(item){
    const answer=String(item.answer||'').trim().toLowerCase();
    const vals=[];
    if(Array.isArray(item.distractors)) vals.push(...item.distractors);
    if(Array.isArray(item.incorrect_answers)) vals.push(...item.incorrect_answers);
    if(Array.isArray(item.options)) vals.push(...item.options);
    vals.push(item.option_a,item.option_b,item.option_c,item.option_d);
    const seen=new Set();
    return vals.map(x=>String(x==null?'':x).trim()).filter(x=>{
      const k=x.toLowerCase();
      if(!k||k===answer||seen.has(k))return false;
  ...
  function buildPool(source,level,count,seed){
    ...
    while(out.length<count){
      for(let i=0;i<shuffled.length&&out.length<count;i++){
        ...
        const distractors=authoredDistractors(item);
        out.push({...item,question,answer:item.answer,difficulty:level,order:out.length+1,distractors});
      }
      pass++;
    }
